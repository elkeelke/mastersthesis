...
LCA in the energy sector often focuses primarily on impacts related to climate change and land use. This thesis aims to contribute to a more comprehensive understanding of the full range of biodiversity impacts associated with energy production infrastructure by identifying relevant factors not yet represented in LCA, attempting to quantifying their magnitude based on remote sensing data, and exploring how that can be incorporated into LCA frameworks at national and global scales.
...
-   Identify certain types of large energy infrastructure (e.g., reservoir dams) or side effects caused by energy production related activities (e.g., oil spills, methane leaks, coal mining pit lakes) that have an impact on biodiversity that is currently underrepresented in LCA because of missing data and/methods
-   Investigate the potential of remote sensing data to provide the information needed to capture these impacts
-   Develop a generalized approach for using such data to improve the coverage of regionalized life cycle inventory data for biodiversity related indicators
-   Describe how the generalized approach can be rolled up into high-level assessments on a regional or global level

The exact research questions are not yet defined.
...
This section gives an overview of what can be expected in the thesis chapter on gap analysis.
...
Existing LCA in the energy sector mostly focus on the climate change impact category. The motivation for the research is to expand the LCA to all biodiversity related impact categories to provide a full picture of the environmental impact of the energy sector as well as better ways to compare the different means of energy production against each other.
